'''
AdvancedNetwork.py: The advanced network.
...
class StackedGRUNetwork:
    ...
    def _trainBatch(self, batch_data, batch_reward):
        ...
        hidden_sequence = np.zeros((times_num, self.network.nlayers, self.network.batch_size, self.network.hid_dim))
        ...
        for i in range(times_num):
            cur_time = np2tensor(batch_data[i:i + 1, :, :],
                                 cuda_enabled=self.cuda_enabled)  # current time step in the batch
            time_reward = reward_guide[i].cuda() if self.cuda_enabled else reward_guide[i]
            next_time = np2tensor(batch_data[i + 1:i + 2, :, :],
                                  gradient_required=False)  # next time step; used for computing loss
    ...
    def _initHidden(self):
        ...
        # for pertubation
        noise = torch.randn(self.network.nlayers, self.network.batch_size, self.network.hid_dim,
                            requires_grad=True) * self.init_noise_amp
...
class _GRUNetwork(nn.Module):
    ...
    def __init__(self, in_dim, batch_size=1, lr=1e-4, optimizer="Adam", cuda_enabled=False):
        ...
        self.out_dim = in_dim
        self.hid_dim = 128
        self.nlayers = 2 # stacked GRU with two layers
        self.batch_size = batch_size
        # self.bg_noise_amp = bg_noise_amp
        self.cuda_enabled = cuda_enabled
        self.lr = lr
        # Initialize hidden layers and mapping functions between layers
        self.rnn = nn.GRU(self.in_dim, self.hid_dim, num_layers=self.nlayers)  # the hidden layers with GRU
        self.relu = nn.ReLU()
...
if __name__ == '__main__':
    network = StackedGRUNetwork("test_config.json")
    rnn = _GRUNetwork(10)
    print(rnn)