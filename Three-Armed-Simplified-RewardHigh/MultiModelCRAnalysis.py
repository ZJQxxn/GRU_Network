...
if __name__ == '__main__':
    model_cr = np.zeros((2, 9, 40))  # 5 models, 8 intermediate models, 40 trials around th reversla points is analyzed
    for num in range(1, 3):
        print('============ MODEL {} ==========='.format(num))
        # Validate intermediate model
        for index in range(1, 10):
            print('------- intermediate model {}'.format(index))
            filename = 'save_m/model_{}/ThreeArmedRewardHigh-validation-1e6-model{}-NUM{}.hdf5'.format(num, num, index)
            ...
            plt.savefig('cr/ThreeArmedRewardHigh-cr-1e6-model{}-NUM{}.pdf'.format(num, index))
            plt.close()
            model_cr[num-1, index-1, :] = correct_rate
        # Validate the final model
        correct_rate = crAnalyze('ThreeArmedRewardHigh-validation-1e6-model{}-final.hdf5'.format(num))
        model_cr[num-1, -1, :] = correct_rate

    for inter_num in range(9):
        ...
        plt.savefig('cr/average_cr/ThreeeArmedRewardHigh-AverageCR-{}e5.pdf'.format(inter_num+1))
        # plt.show()
