...
if __name__ == '__main__':
    model_cr = np.zeros((2, 15, 40))  # 5 models, 8 intermediate models, 40 trials around th reversla points is analyzed
    for num in range(1, 3):
        print('============ MODEL {} ==========='.format(num))
        # Validate intermediate model
        for index in range(1, 15):
            print('------- intermediate model {}'.format(index))
            filename = 'save_m/model_{}/TwoArmedSimplifySlowReverse-validation-15e6-model{}-NUM{}.hdf5'.format(num, num, index)
            correct_rate = crAnalyze(filename)
            model_cr[num-1, index-1, :] = correct_rate
        # Validate the final model
        correct_rate = crAnalyze('TwoArmedSimplifySlowReverse-validation-15e6-model{}-final.hdf5'.format(num))
        model_cr[num-1, -1, :] = correct_rate

    for inter_num in range(15):
        ...
        trial_num = '{}e5'.format(inter_num) if (inter_num + 1 )<=10 else '{}e6'.format((inter_num+1) / 10)
        plt.title('Correct Rate vs. Trial  [trialNum = {}]'.format(trial_num), fontsize=20)
        ...
        plt.savefig('cr/average_cr/TwoArmedSimplifySlowReverse-AverageCR-{}.pdf'.format(trial_num))
        # plt.show()
