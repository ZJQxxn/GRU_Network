...
def negativeLogLikelihood(param, choices, rewards):
    ...
    for trial in range(1, trials_num):
        ...
        reward_value[choice] = (1 - param['alpha']) * reward_value[choice] + param['alpha'] * reward
        # choice-dependent value C_{t+1}(x) = (1-beta) * C_{t}(x) + beta * Cho_t
        choice_value[choice] = (1 - param['beta']) * choice_value[choice] + param['beta'] * choice
        # overall value V_{t}(x) = gamma * Q_{t}(x) + (1-gamma) * C_{t}(x)
        overall_value[choice] = param['gamma'] * reward_value[choice] + (1-param['gamma']) * choice_value[choice]
        value_trajectory.append(overall_value)
        # negative log likelihood
        weighted_overall_value = param['omega'] * overall_value
